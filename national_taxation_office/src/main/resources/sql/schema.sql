...
create table if not exists `user` (
    ...
    `account` varchar(255) character set utf8mb4 unique not null comment '账户信息',
    ...
    `admin` int default 0 comment '是否为超级管理员'
);
...
create table if not exists `info` (
    ...
    `creator_name` varchar(255) not null comment '创建人名字',
...
create table if not exists `consultation_reply` (
    ...
    `replyer_id` bigint not null comment '咨询回复者ID',
    `content` varchar(1023) not null comment '咨询内容',
    `time` datetime not null comment '创建时间'
...
create table if not exists nt_oa.notification_task
(
    ...
    notify_id bigint       not null,
    constraint notification_record_id_uindex
        unique (id)
);
...
(
    ...
    constraint notification_entry_id_uindex
        unique (id)
);
